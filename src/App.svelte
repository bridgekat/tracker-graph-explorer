...
<script>
  ...
  const legend = $derived.by(() => {
    ...
    if (app.colour === "progress") {
      return TD.STATES.filter((s) => app.base.meta.states[s]).map((s) => ({
        colour: `var(--st-${s})`,
        label: STATE_LABEL[s],
      }));
    }
    if (app.colour === "kind") {
      return [
        { colour: hueLine(210), label: "theorem" },
        { colour: hueLine(28), label: "definition" },
      ];
    }
    const roots = [];
    ...
    roots.forEach((r) => {
      ...
      if (roots.length > 1)
        out.push({ colour: hueLine(rt.hue), label: rt.label, family: true });
      app.base.topics.forEach((ti) => {
    ...
    if (hidden)
      out.push({ muted: `and ${hidden} more, which the index names` });
    return out;
  });

  const statusNode = $derived(app.status?.node);
  const title = $derived(app.src ? `tracker graph > ${app.src}` : "tracker graph");
  ...
  const BAR =
    "shrink-0 border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900";
...
<ThemeProvider theme={THEME}>
  <div class="flex h-full flex-col" class:dragging>
    <Navbar class="{BAR} px-3 py-2">
      <NavBrand href="##" class="min-w-0 gap-2">
        <span
          class="flex min-w-0 items-baseline gap-1.5 font-mono text-sm whitespace-nowrap"
        >
          <b class="text-gray-950 dark:text-white">tracker graph</b>
          {#if app.src}
            <span class="text-gray-400 dark:text-gray-600">&gt;</span>
            <span id="srcName" class="truncate text-gray-700 dark:text-gray-300"
              >{app.src}</span
            >
          {/if}
        </span>
      ...
      <div class="flex min-w-0 flex-1 items-baseline gap-2 px-4">
        <span
          id="srcSub"
          class="hidden truncate text-xs text-gray-500 sm:inline"
          >{app.srcSub}</span
        >
      </div>
      <div class="flex shrink-0 items-center gap-2">
        ...
        <Button
          ...
          onclick={toggleTheme}
        >
          {#if dark}<SunOutline class="h-4 w-4" />{:else}<MoonOutline
              class="h-4 w-4"
            />{/if}
        </Button>
      </div>
    ...
    <div
      class="{BAR} flex flex-wrap items-center gap-x-4 gap-y-2 overflow-x-auto px-3 py-1.5"
    >
      <div class="flex shrink-0 items-center gap-2">
      ...
      <div class="ms-auto flex shrink-0 items-center gap-1">
        <Button
          ...
          color="alternative"
          aria-label="Zoom out"
          onclick={() => canvas?.zoomBy(1 / 1.3)}
          ><ZoomOutOutline class="h-4 w-4" /></Button
        >
        <span
          id="exZoomLab"
          class="w-11 text-center font-mono text-[11px] text-gray-500"
        ...
        <Button
          ...
          color="alternative"
          aria-label="Zoom in"
          onclick={() => canvas?.zoomBy(1.3)}
          ><ZoomInOutline class="h-4 w-4" /></Button
        >
        <Button
          id="exFit"
          size="xs"
    ...
    <div
      class="flex shrink-0 items-center gap-4 overflow-x-auto border-t border-gray-200 bg-white
    ...
    >
      ...
      >
        {#if app.status?.warn}
          <span class="text-orange-700 dark:text-orange-400"
            >{app.status.warn}</span
          >
        {:else if statusNode}
          <span
            ><b class="tabular-nums text-gray-950 dark:text-white"
              >{fmt(app.status.below)}</b
            > it rests on</span
          >
          <span
            ><b class="tabular-nums text-gray-950 dark:text-white"
              >{fmt(app.status.above)}</b
            > rest on it</span
          >
          {#if statusNode.kind === "group"}
            <span
              ><b class="tabular-nums text-gray-950 dark:text-white"
                >{fmt(statusNode.count)}</b
              > results inside</span
            >
          {/if}
        {:else}
      ...
      >
        {#each legend as item}
          ...
          {:else}
            <span class="flex items-center gap-1.5 whitespace-nowrap">
              ...
              ></i>
              {#if item.family}<b class="text-gray-950 dark:text-white"
                  >{item.label}</b
                >{:else}{item.label}{/if}
            </span>
          {/if}
        {/each}
        <span class="text-gray-500"
          >left to right — a box sits after what it rests on</span
        >
      </div>

      <div
        id="exStats"
        class="ms-auto shrink-0 ps-3 font-mono text-[10.5px] whitespace-nowrap
                             text-gray-500"
      >
        {app.stats}
      </div>

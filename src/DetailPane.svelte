...
<script>
  ...
  const H4 =
    "mt-4 mb-1.5 text-[11px] font-semibold tracking-wide text-gray-500 uppercase";
  const ROW =
...
<div id="exDetail" class="min-h-0 flex-1 overflow-y-auto p-3">
  {#if !app.sel}
    <p class={MUTED}>
      Pick a box on the canvas, or an entry in the index, to read what it says
  ...
  {:else if group}
    <div class="mb-2 flex flex-wrap gap-1.5">
      ...
      {#if group.ready && !group.done}<Badge color="yellow"
          >ready to work on</Badge
        >{/if}
    </div>
    ...
    <div class="mt-3">
      ...
      >
        {#each TD.STATES.filter((s) => group.byState[s]) as s}
          <i
            style="background: var(--st-{s}); width: {(
              (group.byState[s] / group.sub) *
              100
            ).toFixed(2)}%"
      ...
      <div
        class="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-gray-700 dark:text-gray-300"
      >
        {#each TD.STATES.filter((s) => group.byState[s]) as s}
          <span class="flex items-center gap-1.5">
            <i class={DOT} style="background: var(--st-{s})"></i>
            <b class="text-gray-950 dark:text-white tabular-nums"
              >{fmt(group.byState[s])}</b
            >
            {STATE_LABEL[s]}
          </span>
        {/each}
    ...
    <div class="mt-3 flex flex-wrap gap-1.5">
      ...
      >
        {app.open.has(group.i)
          ? "Close it"
          : group.isModule
            ? "Show its declarations"
            : "Open this group"}
      </Button>
      {#if !group.isModule}
        <Button
          size="xs"
          color="alternative"
          onclick={() => openAllUnder(group.i)}>Open all the way down</Button
        >
      {/if}
      <Button
        size="xs"
        color="alternative"
        onclick={() => setCone({ t: 0, i: group.i })}>Neighbourhood</Button
      >
    </div>
  ...
  {:else}
    ...
    <h3
      class="font-mono text-base font-semibold break-all text-gray-950 dark:text-white"
    >
      {decl.label}
    </h3>
    ...
    <div
      class="mt-2 flex items-start gap-2 rounded border border-gray-200 dark:border-gray-800 bg-gray-100 dark:bg-gray-800 p-2"
    >
      <code
        class="min-w-0 flex-1 font-mono text-[11px] break-all text-gray-700 dark:text-gray-300"
        >{decl.id}</code
      >
      <Button
        size="xs"
    ...
    <div class="mt-3 flex flex-wrap gap-1.5">
      <Button
        size="xs"
        color="alternative"
        onclick={() => setCone({ t: 1, i: decl.i })}>Neighbourhood</Button
      >
      <Button
        size="xs"
        color="alternative"
    ...
    {#if decl.wrong}<h4 class="{H4} !text-orange-700 dark:!text-orange-400">
        Marked wrong
      </h4>
      <Prose text={decl.wrong} />{/if}
    {#if decl.deprecated}<h4
        class="{H4} !text-orange-700 dark:!text-orange-400"
      >
        Deprecated
      </h4>
      <Prose text={decl.deprecated} />{/if}
    {#if decl.desc}<h4 class={H4}>Statement</h4>
      <Prose text={decl.desc} />{/if}
    {#if decl.source}
      <h4 class={H4}>Source</h4>
      <p
        class="font-mono text-[11px] break-all text-gray-700 dark:text-gray-300"
      >
        {decl.source}
      </p>
    {/if}
    ...
    {:else}
      <p class={MUTED}>
        Nothing in the plan; it stands on the ambient library alone.
      </p>
    {/if}
  ...
  <div class="flex flex-col">
    {#each items.slice(0, DEP_CAP) as di}
      <button class={ROW} onclick={() => select({ t: 1, i: di }, true)}>
        ...
        <span class="truncate font-mono" title={B.decl[di].id}
          >{B.decl[di].label}</span
        >

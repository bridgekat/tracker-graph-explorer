    ...
    const c = createCanvas({
      ...
      on: {
        ...
        tip: (t) => {
          ...
          const w = 340,
            h = 24 + t.rows.length * 18;
          app.tip = {
            rows: t.rows,
            left: Math.max(
              8,
              t.x + 16 + w > innerWidth - 8 ? t.x - w - 16 : t.x + 16,
            ),
            top: Math.max(
              8,
              t.y + 16 + h > innerHeight - 8 ? t.y - h - 16 : t.y + 16,
            ),
          };
        },
      },
    ...
    >
      <Card class="max-w-lg p-5">
        ...
        <p class="mt-2 text-xs text-gray-700 dark:text-gray-300">
          That is a lot to draw at once, and it will be a hairball rather than a
          diagram. Opening one group at a time, or picking a declaration and
          switching to Neighbourhood, usually answers the question faster.
        </p>
        <div class="mt-4 flex flex-wrap gap-2">
          <Button size="xs" color="alternative" onclick={drawAnyway}
            >Draw it anyway</Button
          >
          <Button size="xs" onclick={collapseToModules}
            >Open to modules instead</Button
          >
        </div>
      </Card>
    </div>
      ...
      <Card class="max-w-xl p-5">
        ...
        <p class="mt-2 text-xs text-gray-700 dark:text-gray-300">
          ...
          <code class="rounded bg-gray-100 dark:bg-gray-800 px-1 font-mono"
            >tracker graph</code
          > anywhere on this page, or open it, and this draws the project's dependency
          graph — starting at its areas, and opening a group at a time down to the
          individual declaration. Nothing is uploaded; the file never leaves the
          browser.
        </p>
        <p class="mt-2 text-[11px] text-gray-500">
          <code class="rounded bg-gray-100 dark:bg-gray-800 px-1 font-mono"
            >lake build &amp;&amp; lake exe tracker graph &gt; graph.json</code
          >, then open it here; or publish this page beside one and load it with
          <code class="rounded bg-gray-100 dark:bg-gray-800 px-1 font-mono"
            >index.html?graph=graph.json</code
          >.
        </p>

        <div class="mt-4 flex flex-wrap gap-2">
          <Button id="emptyOpen" size="xs" onclick={() => fileInput.click()}
            >Open graph JSON</Button
          >
          <Button
            id="pasteBtn"
            size="xs"
        ...
        {#if pasteOpen}
          ...
          <Button
            ...
            size="xs"
            class="mt-2"
            onclick={() => readText(pasteText.trim(), "pasted JSON")}
            >Use this JSON</Button
          >
        {/if}

        {#if app.error}
          <Alert id="srcErr" color="red" class="mt-3 !text-xs"
            >{app.error}</Alert
          >
        {/if}
